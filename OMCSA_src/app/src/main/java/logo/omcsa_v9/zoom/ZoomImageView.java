...
public class ZoomImageView extends ImageView implements ZoomEngine.Listener, ZoomApi {
    ...
    public ZoomImageView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        float maxZoom = a.getFloat(R.styleable.ZoomEngine_maxZoom, 5);
    ...
    public void setOnListener( ZoomEngine.Listener listener)
    {
        if(listener != null)
        {
            mEngine.setListener(listener);
        }
        else
        {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if( getEngine().getRealZoom() <= 1.05f && ev.getPointerCount() == 1) return true;
    ...
    public void onUpdate(ZoomEngine helper, Matrix matrix) {
        mMatrix.set(matrix);
        setImageMatrix(mMatrix);
        //setScaleX(helper.getRealZoom());
        //setScaleY(helper.getRealZoom());
        //awakenScrollBars();
    }
